...
**Google Scholar Integration (currently disabled):** A GitHub Actions workflow (`.github/workflows/google_scholar_crawler.yaml`) runs `google_scholar_crawler/main.py`, writing citation data as JSON to the `google-scholar-stats` branch. `_includes/fetch_google_scholar_stats.html` fetches this data at page load via CDN and fails silently if it is absent.

The daily schedule is commented out and the `google-scholar-stats` branch does not exist: the owner's Scholar profile is private, and `scholarly` can only read public profiles. To re-enable, make the profile public, set `GOOGLE_SCHOLAR_ID` to the real `user=` value, and uncomment the `schedule:` block.

Note that `scholarly==1.7.11` is unmaintained and pins none of its own dependencies; `requirements.txt` pins `bibtexparser` and `httpx` because new majors of both broke the crawl. If it breaks again, prefer a full `pip freeze` lockfile over another targeted pin.
